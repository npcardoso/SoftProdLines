...
import stats.LogReader;
import stats.LogReader.COV;

/**
 * Implement the concept of configuration: a list of features. Represents a
...
public class Configuration {
  ...
  public Configuration(String[] features) {
    ...
    for (String f : features) {
      if (f.equals("0"))
        configuration.add(LogReader.COV.Disabled0);
      else if (f.equals("1"))
        configuration.add(LogReader.COV.Enabled1);
      else
        configuration.add(LogReader.COV.Untouched);
    }
  }
  ...
  /**
   * @return a configuration represented by a logical formula: a conjunction of
   *         literals.
   */
  public String getFc() {
  ...
  public List<String> getFcLiteral() {
    ...
    for (int i = 0; i < configuration.size(); i++) {
      ...
      if (!feature.equals("?")) {
        int index = i + 1;
        expression.add(feature.equals("1") ? ("f" + index)
            : ("(not f" + index + ")"));
      }
    }
    return expression;
