...
public class Example_Paulo2 {
  ...
  public void testCOMPRESS__() throws IOException{
    ZipMeVariables.getSINGLETON().setCOMPRESS___(true);
    DeflaterOutputStream dos = new DeflaterOutputStream(new FileOutputStream(new File(
        homeDir + "/files/src.zip")));
    dos.deflate();
  ...
  public void testDERIVATIVE_COMPRESS_GZIP___() throws IOException{
    ...
    GZIPOutputStream os = new GZIPOutputStream(new FileOutputStream(new File (homeDir + "/files/file.gz")));
    byte[] b1 = os.buf;
  ...
  public void testDERIVATIVE_EXTRACT_CRC___() throws IOException{
    ...
    ZipMeVariables.getSINGLETON().setCOMPRESS___(true);
    ZipOutputStream zos = new ZipOutputStream(new FileOutputStream(new File (homeDir + "/files/Teste.txt.zip")));
    zos.putNextEntry(new ZipEntry(homeDir + "/files/Teste.txt.zip"));
    zos.write(new byte[32]);
    ZipInputStream zis = new ZipInputStream(new FileInputStream(new File (homeDir + "/files/Teste.txt.zip")));
