 ...
 * etc. It includes support for STORED and DEFLATED entries. This class is not
 * thread safe.
 * 
...
public class ZipOutputStream extends DeflaterOutputStream implements
	...
	/**
	 * Sets default compression method. If the Zip entry specifies another
	 * method its method takes precedence.
	 * 
	 * @param method
	 *            the method.
	 * @exception IllegalArgumentException
	...
	/**
	 * Write a long value as an int. Some of the zip constants are declared as
	 * longs even though they fit perfectly well into integers.
	 */
	...
	/**
	 * Starts a new Zip entry. It automatically closes the previous entry if
	 * present. If the compression method is stored, the entry must have a valid
	 * size and crc, otherwise all elements (except name) are optional, but must
	 * be correct if present. If the time is not set in the entry, the current
	 * time is used.
	 * 
	 * @param entry
	 *            the entry.
	...
	public void putNextEntry(ZipEntry entry) throws IOException {
		if (ZipMeVariables.getSINGLETON().isCOMPRESS___()){
			...
			curMethod = method;
			writeLeInt(LOCSIG);
			writeLeShort(method == STORED ? ZIP_STORED_VERSION
	...
	/**
	 * Finishes the stream. This will write the central directory at the end of
	 * the zip file and flush the stream.
	 * 
	 * @exception IOException
	 *                if an I/O error occured.
	 */
	public void finish() throws IOException {
		if (ZipMeVariables.getSINGLETON().isCOMPRESS___()){
			...
			while (e.hasMoreElements()) {
				...
				out.write(comment);
				numEntries++;
				sizeEntries += CENHDR + name.length + extra.length
						+ comment.length;
			}
			writeLeInt(ENDSIG);
			writeLeShort(0);
	...
	protected void hook42() throws IOException {
		...
		if (ZipMeVariables.getSINGLETON().isDERIVATIVE_COMPRESS_CRC___()){
			 if (curEntry.getCrc() < 0)     curEntry.setCrc(crc.getValue());
				 ...
				 if (curEntry.getCrc() != crc.getValue())     
					 throw new ZipException("crc was " + toHexString(crc.getValue()) + 
							 				", but I expected "+ 
