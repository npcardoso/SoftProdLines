...
class DeflaterEngine implements DeflaterConstants {
	 ...
	 */
	byte[] window;
	int strategy, max_chain, max_lazy, niceLength, goodLength;
	/**
	 ...
	 * and lookahead. OUT assertions: strstart + lookahead <= 2*WSIZE lookahead
	...
	private void fillWindow() {
		if (ZipMeVariables.getSINGLETON().isCOMPRESS___() ){
			...
			while (lookahead < DeflaterConstants.MIN_LOOKAHEAD
				...
				System.arraycopy(inputBuf, inputOff, window, strstart
						+ lookahead, more);
				this.hook28(more);
				inputOff += more;
				totalIn += more;
	 ...
	 * at strstart. Preconditions: strstart + MAX_MATCH <= window.length.
	 * 
	 * @param curMatch
	 */
	...
	private boolean deflateStored(boolean flush, boolean finish) {
		if (ZipMeVariables.getSINGLETON().isCOMPRESS___() ){
			...
			if ((storedLen >= DeflaterConstants.MAX_BLOCK_SIZE)
					|| (blockStart < WSIZE && storedLen >= MAX_DIST) || flush) {
				...
				if (DeflaterConstants.DEBUGGING)
					System.err.println("storedBlock[" + storedLen + ","
							+ lastBlock + "]");
				huffman.flushStoredBlock(window, blockStart, storedLen,
						lastBlock);
		...
		if (ZipMeVariables.getSINGLETON().isCOMPRESS___() ){
			...
			while (lookahead >= MIN_LOOKAHEAD || flush) {
				...
				if (lookahead >= MIN_MATCH && (hashHead = insertString()) != 0
					...
					boolean full = huffman.tallyDist(strstart - matchStart,
							matchLen);
					lookahead -= matchLen;
			...
			while (lookahead >= MIN_LOOKAHEAD || flush) {
				...
				if (lookahead >= MIN_MATCH) {
					...
					if (strategy != Deflater.HUFFMAN_ONLY && hashHead != 0
							...
							&& findLongestMatch(hashHead)) {
						if (matchLen <= 5
								&& (strategy == Deflater.FILTERED || (matchLen == MIN_MATCH && strstart
										- matchStart > TOO_FAR))) {
							matchLen = MIN_MATCH - 1;
						}
					}
				...
				if (prevLen >= MIN_MATCH && matchLen <= prevLen) {
					if (DeflaterConstants.DEBUGGING) {
						for (int i = 0; i < matchLen; i++) {
							if (window[strstart - 1 + i] != window[prevMatch
									+ i])
								throw new Error();
						}
					}
	...
	public boolean deflate(boolean flush, boolean finish) {
		if (ZipMeVariables.getSINGLETON().isCOMPRESS___() ){
			...
			do {
				...
				if (DeflaterConstants.DEBUGGING)
					System.err.println("window: [" + blockStart + ","
							+ strstart + "," + lookahead + "], " + comprFunc
							+ "," + canFlush);
				switch (comprFunc) {
				case DEFLATE_STORED:
